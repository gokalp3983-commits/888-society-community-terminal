...
async function rank(wallet){
  const d=await getJson(`/whales/api/rank?wallet=${encodeURIComponent(wallet)}`);
  if(d.excluded){
    block(`
...
async function whaleProfile(rankNumber){
  const d=await getJson(`/whales/api/whale/${encodeURIComponent(rankNumber)}`);
...
async function activityDashboard(){
  const d=await getJson("/whales/api/activity?hours=24");
...
async function whales12(){
  ...
  try{
    d=await getJson("/whales/api/whales12");
  }catch(error){
...
async function traders12(){
  const d=await getJson("/whales/api/traders12");
...
async function infrastructure(){
  const d=await getJson("/whales/api/infrastructure");
...
async function execute(command){
  ...
  try{
    if(lower==="help"){
      ...
      const data=await getJson("/whales/api/whales?limit=30");
      progress.remove();
      ...
      const data=await getJson("/whales/api/whales?limit=250");
      progress.remove();
      ...
      const d=await getJson("/whales/api/activity?hours=24");progress.remove();
      traderTable(d.topAccumulators,"Top 10 Net Accumulators — 24h","both");
    }
      ...
      const d=await getJson("/whales/api/whales?limit=250");
      progress.remove();
      ...
      const d=await getJson("/whales/api/stats");
      progress.remove();
...
async function refreshMarketPanel(){
  ...
  try{
    const d=await getJson("/whales/api/market");
