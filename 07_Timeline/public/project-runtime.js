...
(() => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll("[data-project-home]").forEach((e) => {
      e.href = "/";
      e.title = `Return to ${c.project.name} Community Portal`;
      if (!e.dataset.homeConfirmBound) {
        e.addEventListener("click", (event) => {
          if (!window.confirm("Return to the main Community Portal landing page?")) event.preventDefault();
        });
    ...
    document.querySelectorAll("[data-project-version]").forEach((e) => {
      e.textContent = `${c.project.name} Community Portal`;
    });
