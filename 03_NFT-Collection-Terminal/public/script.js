...
function writeBoot(html) {
  ...
  line.className = "line";
  line.innerHTML = html || "&nbsp;";
  if (elements.boot && !elements.boot.hidden) elements.boot.appendChild(line);
}

function renderBoot(lines) {
  if (!elements.boot || elements.boot.hidden) return;
...
async function boot() {
  ...
  renderBoot([
    '[ <span class="orange">CONNECTING</span> ] ' +
      `Initializing ${CFG.project.name} NFT Portal...`,
    '[ <span class="orange">CONNECTING</span> ] NFT portal module loading...',
  ...
  if (!isPreMint()) {
    try {
      const response = await fetch("/nft/api/mint-stats", {
  ...
  renderBoot([
    '[ <span class="green">OK</span> ] ' +
      (mobileBoot ? 'NFT Portal ready.' : `${CFG.project.name} NFT Portal initialized.`),
    mobileBoot ? '[ <span class="green">OK</span> ] NFT module ready.' : '[ <span class="green">OK</span> ] NFT portal module loaded.',
...
async function refreshMarketPanel() {
  ...
  const collectionTask = (async () => {
    try {
      const response = await fetch("/nft/api/collection-stats", {
        headers: { Accept: "application/json" },
      });
...
function updatePostMintHolderSummary(){
  ...
  elements.nftWhalesSummary.innerHTML =
    `${uniqueText} unique holders · Largest holder: ${largestText}`;
}
...
function setDisconnectedMintStats(){
  ...
  elements.mintedCount.textContent = "— / 8561";
...
function renderMintStats(stats){
  ...
  elements.mintedCount.textContent =
    `${stats.minted ?? "—"} / ${stats.totalSupply ?? Number("8561")}`;
...
async function refreshMintStats(){
  if(isPreMint()){
    ...
    elements.progressFill.style.width = "0%";
    elements.mintedCount.textContent = "0 / 8561";
    elements.remainingCount.textContent = "8561";
    ...
    applyPostMintCompactMode({
      status: "UPCOMING",
      minted: 0,
      totalSupply: Number(CFG?.nft?.supply) || Number("8561"),
      progressPercent: 0,
    });
    return;
  }
...
async function refreshFloorTrend(){
  ...
  try{
    const response = await fetch("/nft/api/floor-trend", {
      headers: { Accept: "application/json" },
    });
...
async function refreshNftSales(){
  ...
  try{
    const response = await fetch("/nft/api/nft-sales", {
      headers: { Accept: "application/json" },
    });
    const data = await response.json();
...
// CTB Chapter 21A — interactive NFT Portal commands.
...
async function nftCmdMint(){
  const [d,intel]=await Promise.all([nftCommandJson("/nft/api/mint-stats"),nftCommandJson("/nft/api/mint-intelligence",true).catch(()=>({connected:false}))]);
...
async function nftCmdWhales(){
  const d=await nftCommandJson("/nft/api/nft-postmint");
...
async function nftCmdMinters(){
  const d=await nftCommandJson("/nft/api/mint-intelligence",true);
...
async function nftCmdActivity(){
  const d=await nftCommandJsonWithWarmRetry("/nft/api/nft-activity");if(!d.connected){nftCommandBlock("NFT ACTIVITY",[["Status",d.warming?"On-chain activity cache is warming — retry shortly.":"On-chain activity unavailable"]]);return;}
...
async function nftCmdWallet(address){
  ...
  const d=await nftCommandJson(`/nft/api/nft-whales?address=${encodeURIComponent(address)}`);const wallet=d.wallet;
...
async function nftCmdRetention(){
  const d=await nftCommandJson("/nft/api/nft-postmint",true);
...
async function nftCmdSales(){
  const d=await nftCommandJson("/nft/api/nft-sales",true);if(!d.connected){nftCommandBlock("SALES ANALYTICS",[["Status",d.requiresApiKey?"OpenSea API key required":"UNAVAILABLE"]]);return;}
...
async function nftCmdPulse(){
  const [activity,whales,sales,postmint]=await Promise.all([
    nftCommandJsonWithWarmRetry("/nft/api/nft-activity").catch(()=>({connected:false})),
    nftCommandJson("/nft/api/nft-whales",true).catch(()=>({})),
    nftCommandJson("/nft/api/nft-sales",true).catch(()=>({connected:false})),
    nftCommandJson("/nft/api/nft-postmint",true).catch(()=>({connected:false}))
  ]);
  const rows=[
...
async function nftCmdRefresh(){await Promise.allSettled([refreshMarketPanel(),refreshMintStats(),refreshNftSales(),refreshNftWhales(),refreshCollectionStats(),refreshFloorTrend()]);nftCommandBlock("REFRESH",[["Status","NFT portal data refresh completed"]]);}
