...
function phaseSessionKey(id){ return `888-society-phase-live-seen:${id}`; }
...
function compactPhaseInfo(){
  return phases.map((phase,index)=>{
    const name=String(phase.name||phase.label||`Phase ${index+1}`).trim();
    const start=phaseTimeText(phase.startsAt);
    const end=phaseTimeText(phase.endsAt);
    const price=String(phase.price||"—");
    const limit=String(phase.limit||"—");
    return `Phase ${index+1} (${name}) · ${start}${end!=="Not configured"?`–${end}`:""} · Mint ${price} · Wallet limit ${limit}`;
  }).join("  |  ");
}
...
function showPhaseModal(phase){
  ...
  $("phaseLiveText").textContent = `${phase.label} has started. Visit the NFT Portal for live activity, or stay here to follow the remaining mint schedule.`;
  modal.hidden = false;
}

function setOverallComplete(){
  overallLiveSet = true;
  const pageState = document.querySelector(".multi-pre-mint-state");
  if (pageState) { pageState.textContent = "MINT COMPLETE"; pageState.classList.remove("is-live"); pageState.classList.add("is-complete"); }
  ...
  $("mintCommand").innerHTML =
    `<span class="green">[ COMPLETE ]</span> ${responsiveStatusCopy("888 SOCIETY mint schedule has concluded.", "Mint complete.")}`;
  $("mintReady").innerHTML =
    `<span class="green">[ READY ]</span> ${responsiveStatusCopy("NFT Portal is tracking collection activity.", "Tracking active.")}`;
  ...
  if (completionModal) {
    ...
    $("phaseLiveTitle").textContent = "THE 888 SOCIETY MINT IS COMPLETE";
    $("phaseLiveText").textContent =
      "The mint schedule has concluded. Visit the NFT Portal for collection activity or view the collection on OpenSea.";
...
function setOverallLive(now = Date.now()){
  const livePanel = $("launchLivePanel");
  const pageState = document.querySelector(".multi-pre-mint-state");
  if (pageState) { pageState.textContent = "MINT LIVE"; pageState.classList.remove("is-complete"); pageState.classList.add("is-live"); }
  ...
  if (!overallLiveSet) {
    ...
    const terminalLine = $("postMintTerminalLine");
    if (terminalLine) terminalLine.hidden = false;
    $("mintCommand").innerHTML = `<span class="green">[ LIVE ]</span> ${responsiveStatusCopy("888 SOCIETY mint schedule is underway.", "Mint active.")}`;
...
function setOverallPending(){
  const pageState = document.querySelector(".multi-pre-mint-state");
  if (pageState) {
    pageState.textContent = "MINT HAS NOT STARTED";
    pageState.classList.remove("is-live","is-complete");
  }
  ...
  if (livePanel) {
    livePanel.hidden = false;
    livePanel.innerHTML = `
      <div class="countdown-box pre-mint-status-box">
        <div class="countdown-value">MINT HAS NOT STARTED</div>
      </div>`;
  }
}
...
function showCurrentPhaseModal(phase){
  ...
  $("phaseLiveTitle").textContent = `“${phase.name}” IS NOW LIVE`;
  $("phaseLiveText").textContent =
    `${phase.label} has started. Visit the NFT Portal for live activity, or stay here to follow the remaining mint schedule.`;
  modal.hidden = false;
}
...
function handleTerminalEntry(event){
  ...
  if (finalPhase?.endMs && now >= finalPhase.endMs){
    window.location.href = "/nft/terminal";
    return;
  }
