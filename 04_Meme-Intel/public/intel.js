...
function echo(cmd) {
  const line = document.createElement("div");
  line.className = "command-echo command-output-start";
  line.tabIndex = -1;
  line.innerHTML = `<span class="prompt-echo">${PROMPT}</span> ${esc(cmd)}`;
  history.appendChild(line);
  return line;
}
...
async function activity(hours = 12) { return api(`/intel/api/activity?hours=${hours}`); }
function warming(error) {
  if (error.warming) {
    print(`<div class="intel-block"><div class="intel-title">[ CACHE BUILDING ]</div><div class="intel-note">The intelligence cache is being assembled in the background. The portal must fetch and classify multiple pages of holder and transfer data while pacing requests to avoid Blockscout rate limits. A cold Render start can therefore take a few minutes. Use <strong>status</strong> to check progress, then retry the command when the cache is READY.</div></div>`, "status-warning");
    return;
  }
...
async function marketHeader() {
  try {
    const m = await api("/intel/api/market");
    const set = (id, value) => document.getElementById(id).textContent = value;
    const numericPrice = Number(m.priceUsd);
    const priceText = Number.isFinite(numericPrice) && numericPrice > 0
      ? `$${numericPrice.toFixed(5)} USD / ${m.priceEth ?? "—"} ETH`
      : "UNAVAILABLE";
    set("marketPrice", priceText);
...
async function cmdStatus() {
  try {
    const s = await api("/intel/api/cache-status");
...
async function cmdScan() {
  try {
    const [m, s, a] = await Promise.all([api("/intel/api/market"), api("/intel/api/stats"), activity(12)]);
...
async function cmdHolders() {
  try {
    const s = await api("/intel/api/stats");
    const st=s.stats||{};
...
async function cmdRisk() {
  try {
    const [m,s,a]=await Promise.all([api("/intel/api/market"),api("/intel/api/stats"),activity(24)]);
...
async function cmdPulse() {
  try {
    const [m,a]=await Promise.all([api("/intel/api/market"),activity(12)]);
...
function cmdHelp(){
  block("AVAILABLE COMMANDS", [
    ...
    ["clear", "Clears the current intelligence output."],
...
boot.innerHTML = mobileBoot
  ? `<div class="boot-line visible">[ <span class="green">OK</span> ] Intel Portal ready.</div><div class="boot-line visible">[ <span class="green">OK</span> ] Market services connected.</div><div class="boot-line visible">[ <span class="green">OK</span> ] Signals loaded.</div><div class="boot-line visible">[ <span class="green">READY</span> ] Intel synchronized.</div>`
  : `<div class="boot-line visible">[ <span class="green">OK</span> ] Initializing ${CFG.project.name} Meme Intelligence Portal</div><div class="boot-line visible">[ <span class="green">OK</span> ] Connecting to market intelligence services</div><div class="boot-line visible">[ <span class="green">OK</span> ] Loading current intelligence signals</div><div class="boot-line visible">[ <span class="green">READY</span> ] Intelligence portal synchronized</div>`;
