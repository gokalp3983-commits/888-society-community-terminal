...
async function start(){
  ...
  setSignal("nft",nftOn?"CONFIGURED":"NOT ENABLED",nftOn?`${CFG.nft?.collectionName||CFG.project.name+" NFT"} is connected to this Community Portal.`:"NFT Portal is not enabled for this project.",nftOn?"positive":"");
  const signals=[];
  try{const m=await api("/pulse/api/market");const s=marketSignal(m);setSignal("market",...s);signals.push(s)}catch(e){setSignal("market","UNAVAILABLE",e.message,"negative")}
  try{const sdata=await api("/pulse/api/stats");const s=holderSignal(sdata);setSignal("holder",...s);signals.push(s)}catch(e){setSignal("holder","UNAVAILABLE",e.message,"negative")}
  try{const a=await api("/pulse/api/activity?hours=24");const w=whaleSignal(a),f=freshSignal(a);setSignal("whale",...w);setSignal("fresh",...f);signals.push(w,f)}catch(e){setSignal("whale","BUILDING","Recent classified whale activity is still building.","");setSignal("fresh","BUILDING","Fresh-wallet activity is still building.","")}
...
// Shared portal navigation behavior: always begin at top and expose the standard back-to-top control.
if("scrollRestoration" in history) history.scrollRestoration="manual";
window.addEventListener("pageshow",()=>window.scrollTo(0,0));
window.addEventListener("load",()=>window.scrollTo(0,0));
const backToTop=document.getElementById("backToTop");
if(backToTop){
  const syncBackToTop=()=>backToTop.classList.toggle("visible",window.scrollY>420);
